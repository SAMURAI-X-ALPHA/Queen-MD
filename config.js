...
global.ownername = process.env.OWNER_NAME === undefined ? "𝐌𝐈𝐇𝐈𝐑𝐔𝐊 𝐃𝐈𝐋𝐒𝐇𝐀𝐍" : process.env.OWNER_NAME
let fake = process.env.OWNER_NUMBER === undefined ? '94761091578' : process.env.OWNER_NUMBER
global.owner = [`${Mihiruk Dilshan}`]
global.ownernumber = [`${0766969406}`]
global.premium = [`${fake}`]
global.anticall = process.env.ANTICALL === undefined ? 'false' : process.env.ANTICALL
global.packname = process.env.PACK_NAME === undefined ? 'Queen-MD Mihiruk' : process.env.PACK_NAME
global.author = process.env.STICKER_AUTHOR_NAME === undefined ?'Mihiruk' : process.env.STICKER_AUTHOR_NAME
